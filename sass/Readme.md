...
folder are mapped to the application's root namespace, 'cascoFront'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "cascoFront"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'cascoFront' in the same way
as `"cascoFront/sass/src"`.
